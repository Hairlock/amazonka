...
-- |
-- Module      : Amazonka.SageMakerMetrics.Types
...
defaultService =
  Core.Service
    { Core.abbrev = "SageMakerMetrics",
      Core.signer = Sign.v4,
      Core.endpointPrefix = "metrics.sagemaker",
      Core.signingName = "sagemaker",
      Core.version = "2022-09-30",
      Core.s3AddressingStyle = Core.S3AddressingStyleAuto,
      Core.endpoint = Core.defaultEndpoint defaultService,
      Core.timeout = Prelude.Just 70,
      Core.check = Core.statusSuccess,
      Core.error = Core.parseJSONError "SageMakerMetrics",
      Core.retry = retry
    }
  where
    retry =
