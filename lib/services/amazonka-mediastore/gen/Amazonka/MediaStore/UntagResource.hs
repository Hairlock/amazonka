...
--
-- 'resource', 'untagResource_resource' - The Amazon Resource Name (ARN) for the container.
--
-- 'tagKeys', 'untagResource_tagKeys' - A comma-separated list of keys for tags that you want to remove from the
-- container. For example, if your container has two tags
-- (customer:CompanyA and priority:High) and you want to remove one of the
-- tags (priority:High), you specify the key for the tag that you want to
-- remove (priority).
...
instance Data.ToHeaders UntagResource where
  toHeaders =
    Prelude.const
      ( Prelude.mconcat
          [ "X-Amz-Target"
              Data.=# ( "MediaStore_20170901.UntagResource" ::
                          Prelude.ByteString
                      ),
            "Content-Type"
